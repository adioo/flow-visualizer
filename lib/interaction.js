...
module.exports = (scope, inst, options) => {
    ...
    network.on('click', event => {
        ...
        setTimeout(() => {
            if (t - eventTime > threshold) {
                expandCollapse(scope, inst, event);
            }
        }, threshold);
    });
    ...
    network.on('hold', event => {
        eventTime = new Date();
        contextMenu(scope, inst, event);
    });

    // 2x Click > show detail form
    network.on('doubleClick', event => {
        eventTime = new Date();
        const node = event.nodes.length ? inst.nodes.get(event.nodes[0]) : null;

        if (!node) {
            return;
        }

        scope.flow(inst._name + '/detailView').write({node: node});
    });
    ...
    network.on('afterDrawing', (ctx) => {
        ...
        let nodeIds = Object.keys(inst.contextMenus);

        nodeIds.forEach(nodeId => {
            let node = inst.nodes.get(nodeId);
            if (!node || !inst.types[node.type]) {
                return;
            }

            let actions = inst.types[node.type].actions;
            ...
            let menuX = inst.network.getPositions([nodeId])[nodeId].x;
            ...
            inst.contextMenus[nodeId] = buttons;
        });
    ...
    canvas.addEventListener('mousemove', event => {
        ...
        Object.keys(inst.contextMenus).forEach(nodeId => {
            let buttons = inst.contextMenus[nodeId];
            buttons.forEach(button => {
    ...
    canvas.addEventListener('mousedown', event => {
        ...
        Object.keys(inst.contextMenus).forEach(nodeId => {
            ...
            buttons.forEach(button => {
                if (mouse.x >= button.x && mouse.x <= button.x + button.width && mouse.y >= button.y && mouse.y <= button.y + button.height) {
                    button.mouseDown();
                    scope.flow(inst._name + '/' + button.event).write({node: button.node});
                }
            });
        });
    }, false);
...
function expandCollapse (scope, inst, event) {
    ...
    if (node.id) {
        let pos = inst.network.getPositions(node.id)[node.id];
        node.x = pos.x;
        node.y = pos.y;
    }

    inst.nodes.update(node);

    scope.flow(inst._name + '/' + node.state).write({node: node});
}

function contextMenu (scope, inst, event) {
    ...
    if (!inst.contextMenus[node.id]) {
        inst.contextMenus[node.id] = {};
    } else {
        delete inst.contextMenus[node.id];
    }
}
