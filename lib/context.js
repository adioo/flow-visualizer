...
exports.toggle = (state, node) => {
    ...
    if (!node || !state.config.nodes.types[node.type]) {
        return;
    }

    // hide context menu
    if (
        (!state.config.nodes.types[node.type].buttons || !state.config.nodes.types[node.type].buttons.length) ||
...
exports.init = (scope, state) => {
    ...
    state.network.on('afterDrawing', (ctx) => {
        ...
        if (!state.contextMenu || !state.config.nodes.types[state.contextMenu.type]) {
            return;
        }
        ...
        if (!node.buttons) {
            ...
            let buttons = [];
            let actions = state.config.nodes.types[node.type].actions;
            actions.forEach(action => {
                ...
                // append generic action data
                if (action.data) {
                    button.data = action.data;
                }
    ...
    state.network.on('click', event => {
        ...
        state.contextMenu.buttons.forEach(button => {
            if (mouse.x >= button.x && mouse.x <= button.x + button.width && mouse.y >= button.y && mouse.y <= button.y + button.height) {
                ...
                scope.flow(button.emit).end(chunk);
            }
        });
    });
};
