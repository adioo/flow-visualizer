...
exports.navigate = function (direction, nodeId) {
    var self = this;
    ...
    if (!nodeId) {
        ...
        let center = self.network.DOMtoCanvas({
            x: canvas.offsetLeft + canvas.width / 2,
            y: canvas.offsetTop + canvas.height / 2
        });
        let distanceToCenter;

        Object.keys(self.nodes._data).forEach(cNodeId => {
            let cNode = self.nodes._data[cNodeId];
            let cPosition = self.network.getPositions(cNode.id);
    ...
    let node = self.nodes._data[nodeId];
    if (!node) {
        return;
    }
    ...
    let parents = [];
    Object.keys(self.edges._data).forEach(edgeId => {
        let cEdge = self.edges._data[edgeId];
    ...
    Object.keys(self.edges._data).forEach(edgeId => {
        ...
        if (cEdge.from === node.id) {
            let cNode = self.nodes.get(cEdge.to);
            if (cNode) {
                children.push(cNode);
            }
        }
    ...
    if (parents.length && self.nodes._data[node.parent]) {
        parents.forEach(parentId => {
            let parent = self.nodes.get(parentId);
            if (parent) {
    ...
    let position = self.network.getPositions(node.id);
    node.x = position[node.id].x;
    node.y = position[node.id].y;

    // check if any children are in navigable area
    let result = findSelectables(direction, node, children);

    // if no result so far look for siblings
    if (!result) {
        result = findSelectables(direction, node, siblings);
    }

    // if still no result look for children but with an increased distance
    if (!result) {
        result = findSelectables(direction, self, node, children.concat(siblings), 50);
    }
