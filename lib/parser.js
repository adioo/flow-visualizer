...
module.exports = function (predicates, triples, types, target, pos, index) {
    ...
    triples.forEach(triple => {
        ...
        Object.keys(config).forEach(category => config[category].forEach(map => {
            ...
            if (index[category][element.id] && index[category][element.id].add) {
                return;
            }
            ...
            if (category === 'nodes') {
                ...
                index.nodes[element.id] = {children: [], out: [], add: true};
            ...
            // add edge to node out
                ...
                map.out && index.nodes[element.from].out.push(element.id);
            }
